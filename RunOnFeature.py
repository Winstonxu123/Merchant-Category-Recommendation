...
#    hist_purchase_date = hist.groupby(['card_id']).agg({'purchase_date': ['max', 'min']})
#    
#    hist_purchase_date.columns = ['refer_' + '_'.join(col).strip() \
#                           for col in hist_purchase_date.columns.values]
#    hist_purchase_date.reset_index(inplace=True)
...
####################### Start ##########################################
extracted_features_train = pd.read_csv('feature/train_data5.csv')
extracted_features_train = reduce_mem_usage(extracted_features_train)
extracted_features_test = pd.read_csv('feature/test_data5.csv')
...
historical = pd.read_csv('data/input/historical_transactions.csv', usecols=['card_id', 'purchase_date'])
new_tran = pd.read_csv('data/input/new_merchant_transactions.csv', usecols=['card_id', 'purchase_date'])
...
use_cols = [col for col in train.columns if col not in 
            ['reference_date', 'hist_purchase_date_max', 'hist_purchase_date_min', 'new_purchase_date_max', 'new_purchase_date_min', 'target', 'card_id', 'outliers', 'first_active_month','histauth_purchase_date_max', 'histauth_purchase_date_min',
...
sub_df.to_csv("./result/submission_test5_referenceDate.csv", index=False)
