...
original_train = outlier(original_train)
...
train['target'] = target
train['outliers'] = original_train['outliers']
...
FOLDS = sklearn.model_selection.KFold(n_splits=10, shuffle=True, random_state=15)
...
for fold_, (trn_idx, val_idx) in enumerate(FOLDS.split(train[use_cols], train['outliers'])):
    train_x, train_y = train[use_cols].iloc[trn_idx], train['target'].iloc[trn_idx]
    valid_x, valid_y = train[use_cols].iloc[val_idx], train['target'].iloc[val_idx]
    ...
    oof_lgb[val_idx] = clf.predict(valid_x, num_iteration=clf.best_iteration)
...
#    feature_importance_df_lgb = pd.concat([feature_importance_df_lgb, fold_importance_df_lgb], axis=0)
    predictions_lgb += clf.predict(test[use_cols], num_iteration=clf.best_iteration) / FOLDS.n_splits
